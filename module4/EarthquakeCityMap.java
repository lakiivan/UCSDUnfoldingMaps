...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		int tx = 50;
		int ty = 125;
		triangle(tx-t_size, ty+t_size, tx, ty-t_size, tx+t_size, ty+t_size);
		fill(0);
		...
		int cx = 50;
		int cy = 150;
		ellipse(cx, cy, c_size, c_size);
		fill(0);
		text("Land Quake", 75, cy);
		
		fill(color(255, 255, 255));
		int r_size = 15;
		int rx = 50;
		int ry = 175;
		rect(rx-r_size/2, ry-r_size/2, r_size, r_size);
		fill(0);
		text("Ocean Quake", 75, ry);
		
		//fill magnitude legend
		fill(0);
		int smy = 225;
		text("Size ~ Maginitude", 50, smy);
	...
	private void printQuakes() 
	{
		...
		List<PointFeature> earthQuakes = ParseFeed.parseEarthquake(this, earthquakesURL);
		HashMap<String, Integer> quakesByCountry = new HashMap<>();
		
		int oceanCounter = 0;
		boolean isFirst = true;
		for(Marker cm : countryMarkers) {
			for(PointFeature eq : earthQuakes) {
				if(isLand(eq) && isInCountry(eq, cm)) {
					String countryName = (String)cm.getProperty("name");
					if(quakesByCountry.containsKey(countryName)) {
						quakesByCountry.put(countryName, (quakesByCountry.get(countryName)+1));
					} else {
						quakesByCountry.put(countryName, 1);
					}
				} 
				
				if(isFirst && !isLand(eq)) {
					oceanCounter++;
				}
			}
			isFirst = false;
		}
		printSortedEQ(quakesByCountry);
	...
	private void printSortedEQ(HashMap<String, Integer> myMap) {
